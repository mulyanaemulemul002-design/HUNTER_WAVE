...
export const PRACTICE_TASKS = {
  "social-quest-wl": [
    { id: "social-x-follow", label: "Follow akun X resmi", detail: "Pastikan handle dan domain berasal dari pengumuman resmi.", steps: ["source"] },
    { id: "social-telegram-join", label: "Join Telegram community", detail: "Baca pinned message dan jangan pernah membagikan seed phrase.", steps: ["rules"] },
    { id: "social-discord-role", label: "Ambil role di Discord", detail: "Cek channel rules sebelum menghubungkan akun apa pun.", steps: ["rules"] },
    { id: "social-safety-check", label: "Cek eligibility & safety", detail: "Task sosial bukan jaminan whitelist atau alokasi.", steps: ["risk"] },
  ],
  "dex-swap": [
    { id: "swap-pair", label: "Pilih pair dan network", detail: "Pastikan aset, network, dan saldo demo sudah sesuai.", steps: ["network"] },
    { id: "swap-quote", label: "Minta quote swap", detail: "Baca amount out, fee, price impact, dan minimum received.", steps: ["quote"] },
    { id: "swap-route", label: "Inspeksi route", detail: "Lihat pool dan contract yang akan digunakan.", steps: ["permission"] },
    { id: "swap-volume", label: "Tetapkan target volume", detail: "Gunakan angka demo untuk memahami notional, bukan untuk transaksi nyata." },
  ],
  defi: [
    { id: "defi-landing", label: "Buka landing dashboard", detail: "Kenali TVL, available liquidity, dan total borrows.", steps: ["asset"] },
    { id: "defi-borrowing", label: "Buka borrowing market", detail: "Bandingkan borrow APY dan health factor.", steps: ["terms"] },
    { id: "defi-reserve", label: "Pilih reserve", detail: "Baca available liquidity dan asset risk sebelum preview.", steps: ["asset"] },
    { id: "defi-exit", label: "Kenali kondisi keluar", detail: "Pahami withdrawal, repayment, atau liquidation condition.", steps: ["exit"] },
  ],
  "meme-launchpad": [
    { id: "meme-tokenomics", label: "Baca tokenomics", detail: "Cek supply, distribusi, unlock, dan admin rights.", steps: ["tokenomics"] },
    { id: "meme-liquidity", label: "Cek liquidity curve", detail: "Lihat sumber liquidity dan aturan exit.", steps: ["liquidity"] },
    { id: "meme-volatility", label: "Review volatility risk", detail: "Popularitas bukan pengganti due diligence.", steps: ["volatility"] },
  ],
  bridge: [
    { id: "bridge-networks", label: "Pilih source & destination", detail: "Pastikan network asal dan tujuan tidak tertukar.", steps: ["networks"] },
    { id: "bridge-asset", label: "Cek asset representation", detail: "Bedakan native asset dengan wrapped representation.", steps: ["asset"] },
    { id: "bridge-finality", label: "Review fee & finality", detail: "Baca ETA, validator, dan kondisi gagal.", steps: ["finality"] },
  ],
  "perps-trading": [
    { id: "perps-side", label: "Pilih Long atau Short", detail: "Tentukan skenario harga sebelum melihat order ticket.", steps: ["margin"] },
    { id: "perps-volume", label: "Tetapkan target notional", detail: "Bandingkan margin dengan volume posisi demo.", steps: ["margin"] },
    { id: "perps-funding", label: "Baca funding rate", detail: "Funding dapat menambah atau mengurangi biaya posisi.", steps: ["funding"] },
    { id: "perps-liquidation", label: "Review liquidation price", detail: "Selalu tahu stop condition sebelum membuka posisi.", steps: ["liquidation"] },
  ],
  nft: [
    { id: "nft-contract", label: "Cek contract & standard", detail: "Pastikan token standard dan contract sesuai collection.", steps: ["contract"] },
    { id: "nft-metadata", label: "Inspeksi metadata", detail: "Periksa lokasi metadata dan traits.", steps: ["metadata"] },
    { id: "nft-ownership", label: "Pahami ownership", detail: "Baca mint, royalty, dan permission.", steps: ["ownership"] },
  ],
  "onchain-interaction": [
    { id: "onchain-target", label: "Verifikasi contract target", detail: "Cek network dan alamat sebelum membaca function.", steps: ["target"] },
    { id: "onchain-parameters", label: "Review parameters", detail: "Bedakan read call dan write call.", steps: ["parameters"] },
    { id: "onchain-receipt", label: "Baca expected receipt", detail: "Kenali event dan status yang mungkin muncul.", steps: ["receipt"] },
  ],
  "smart-contract-activity": [
    { id: "contract-scope", label: "Pilih template contract", detail: "Kenali fungsi dan caller scope template lokal.", steps: ["scope"] },
    { id: "contract-state", label: "Baca state & events", detail: "Hubungkan storage dengan event yang dipancarkan.", steps: ["state"] },
    { id: "contract-tests", label: "Review test scenarios", detail: "Jangan deploy sebelum skenario error dipahami.", steps: ["tests"] },
  ],
  "rwa-special-type": [
    { id: "rwa-issuer", label: "Verifikasi issuer", detail: "Cari issuer dan custodian yang bertanggung jawab.", steps: ["issuer"] },
    { id: "rwa-rights", label: "Baca holder rights", detail: "Periksa transfer restriction dan legal wrapper.", steps: ["rights"] },
    { id: "rwa-evidence", label: "Review reserve evidence", detail: "Audit trail dan redemption bukan sekadar label token.", steps: ["evidence"] },
  ],
};
  ...
  return (
    <div className="border border-white/[0.1] bg-[#0c131e] p-4" data-testid={`task-list-${category.id}`}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[9px] font-bold uppercase tracking-[0.16em] text-white/35">{title}</p>
          <p className="mt-1 text-[10px] text-white/35">{completed}/{tasks.length} tugas demo selesai</p>
        </div>
        <span className="rounded-full border border-emerald-200/20 bg-emerald-200/10 px-2 py-1 text-[9px] font-bold text-emerald-100/75">LOCAL</span>
      </div>
      <div className="mt-3 grid gap-2">
        {tasks.map((task, index) => {
          ...
          return (
            ...
            >
              <span className={`mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-[9px] font-black ${done ? "bg-emerald-200/15 text-emerald-100" : "bg-white/[0.06] text-white/35"}`}>
...
function SocialSimulator({ data, interact }) {
  ...
  const taskRows = [
    { id: "source", label: "Follow X / official account", meta: "@AstraDrop · source verified", status: "Social task" },
    { id: "rules", label: "Join Telegram community", meta: "t.me/astradrop · read pinned rules", status: "Community task" },
    { id: "risk", label: "Read safety & eligibility", meta: "No seed phrase · no guaranteed allocation", status: "Safety check" },
  ];
  return (
    <ProtocolShell category={PRACTICE_CATEGORIES[0]} name="AstraDrop" mark={<Users className="h-4 w-4" />} nav={["Dashboard", "Tasks", "Eligibility"]}>
      <div className="border-b border-white/[0.09] px-4 py-4 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><DemoPill>FICTIONAL CAMPAIGN</DemoPill><h5 className="mt-3 text-2xl font-extrabold tracking-[-0.05em] text-white">Community round</h5><p className="mt-1 text-xs text-white/42">Published by AstraDrop Labs · local dashboard</p></div>
          <div className="grid grid-cols-3 gap-3 sm:flex sm:gap-5"><Metric label="Snapshot" value="24 MAY" /><Metric label="Tasks" value="4" /><Metric label="Status" value="Open" tone="green" /></div>
        </div>
        <div className="mt-5 flex gap-5 overflow-x-auto text-[10px] font-bold text-white/42">
          <button type="button" onClick={() => interact("tab", ["source"], "Overview")} data-testid="button-practice-social-overview" className={`border-b-2 pb-2 ${tab === "Overview" ? "border-sky-200 text-white" : "border-transparent"}`}>Dashboard</button>
          <button type="button" onClick={() => interact("tab", ["rules"], "Tasks")} data-testid="button-practice-social-tasks" className={`border-b-2 pb-2 ${tab === "Tasks" ? "border-sky-200 text-white" : "border-transparent"}`}>Tasks</button>
          <button type="button" onClick={() => interact("tab", ["risk"], "Eligibility")} data-testid="button-practice-social-eligibility" className={`border-b-2 pb-2 ${tab === "Eligibility" ? "border-sky-200 text-white" : "border-transparent"}`}>Eligibility</button>
        </div>
      </div>
      <div className="grid gap-0 lg:grid-cols-[1fr_285px]">
        <div className="p-4 sm:p-6">
          <div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-white/35">{tab === "Tasks" ? "Community task board" : tab === "Eligibility" ? "Eligibility timeline" : "Campaign dashboard"}</p><p className="mt-1 text-xs text-white/42">Setiap status di bawah adalah state latihan lokal.</p></div><DemoPill>TEST ONLY</DemoPill></div>
          {tab === "Tasks" && <div className="mt-4 divide-y divide-white/[0.08] border-y border-white/[0.08]">{taskRows.map((task) => <button type="button" key={task.id} onClick={() => interact("selectedCard", [task.id], task.id)} data-testid={`button-practice-social-${task.id}`} className="flex w-full items-center gap-3 py-4 text-left transition hover:bg-white/[0.03]"><span className={`flex h-7 w-7 items-center justify-center border ${data.steps[task.id] ? "border-emerald-200/35 bg-emerald-200/10 text-emerald-100" : "border-white/[0.12] text-white/35"}`}>{data.steps[task.id] ? <Check className="h-3.5 w-3.5" /> : <span className="text-[10px]">{task.id === "source" ? "01" : task.id === "rules" ? "02" : "03"}</span>}</span><span className="min-w-0 flex-1"><span className="block text-xs font-bold text-white/78">{task.label}</span><span className="mt-1 block text-[10px] text-white/35">{task.meta}</span></span><span className="text-[10px] text-sky-100/60">{task.status}</span><ArrowRight className="h-3.5 w-3.5 text-white/25" /></button>)}</div>}
          {tab === "Eligibility" && <div className="relative mt-5 space-y-5 border-l border-sky-200/25 pl-5">{["Campaign published", "Community tasks open", "Snapshot window", "Eligibility result"].map((item, index) => <div key={item} className="relative"><span className={`absolute -left-[25px] top-0 flex h-4 w-4 items-center justify-center border ${index < 2 ? "border-emerald-200/45 bg-emerald-200/15" : "border-white/20 bg-[#101722]"}`}>{index < 2 && <Check className="h-2.5 w-2.5 text-emerald-100" />}</span><p className="text-xs font-bold text-white/70">{item}</p><p className="mt-1 text-[10px] text-white/35">{index === 3 ? "Task completion is not an allocation guarantee." : "Demo campaign milestone"}</p></div>)}</div>}
          {tab === "Overview" && <div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="border border-white/[0.1] bg-[#0c131e] p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-white/32">Community score</p><p className="mt-3 text-xl font-extrabold text-white/80">3 / 4</p></div><div className="border border-white/[0.1] bg-[#0c131e] p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-white/32">Snapshot</p><p className="mt-3 text-xl font-extrabold text-white/80">24 MAY</p></div><div className="border border-white/[0.1] bg-[#0c131e] p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-white/32">Risk</p><p className="mt-3 text-xl font-extrabold text-amber-100/80">Review</p></div></div>}
        </div>
        <aside className="border-t border-white/[0.09] bg-[#0c131e] p-4 lg:border-l lg:border-t-0"><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-white/35">Announcement drawer</p>{source ? <div className="mt-4"><div className="flex items-center gap-2 text-emerald-100/75"><CheckCircle2 className="h-4 w-4" /><span className="text-[10px] font-bold">Source opened locally</span></div><p className="mt-4 text-xs leading-6 text-white/55">Domain, author, and publish date should agree. Social tasks can be completed without sharing wallet secrets.</p><div className="mt-4 border-t border-white/[0.08] pt-3 font-mono text-[10px] text-sky-100/60">astra.example/campaign</div></div> : <div className="mt-4 border border-dashed border-white/[0.15] p-3 text-[11px] leading-5 text-white/42">Select a task to open its announcement and evidence.</div>}</aside>
      </div>
      <div className="border-t border-white/[0.09] p-4 sm:p-6"><PracticeTaskList category={PRACTICE_CATEGORIES[0]} data={data} interact={interact} title="Community task checklist" /></div>
    </ProtocolShell>
  );
}
...
function DefiSimulator({ data, interact }) {
  const tab = data.simulator?.tab || "landing";
  const selected = data.simulator?.reserve;
  const surface = tab === "borrow" ? "Borrow" : tab === "lp" ? "LP" : tab === "landing" ? "Market overview" : "Supply";
  const reserves = [
    ["Ethereum", "ETH", "2.15M", "1.44%", "2.05%"],
    ["Wrapped ETH", "wETH", "1.31M", "< 0.01%", "1.01%"],
    ["USD Coin", "USDC", "2.31B", "3.65%", "4.36%"],
    ["Test Dollar", "TUSD", "742.8K", "4.72%", "5.18%"],
  ];
  const tabs = ["landing", "supply", "borrow", "lp"];
  return (
    <ProtocolShell category={PRACTICE_CATEGORIES[2]} name="NOVA Finance" mark={<Landmark className="h-4 w-4" />} nav={["Dashboard", "Markets", "Governance", "Savings"]}>
      <div className="bg-[#202735] px-4 py-5 sm:px-6">
        <div className="flex flex-wrap items-end gap-6">
          <div><p className="text-sm font-extrabold text-white/88">Core Instance <span className="ml-1 text-[10px] text-cyan-200/75">V3</span></p><p className="mt-1 text-[10px] text-white/38">Demo market with supply and borrow options</p></div>
          <Metric label="Total market size" value="$24.71B" /><Metric label="Total available" value="$14.19B" /><Metric label="Total borrows" value="$10.00B" />
        </div>
      </div>
      <div className="p-4 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex gap-1 overflow-x-auto border-b border-white/[0.1]">
            {tabs.map((key) => <button type="button" key={key} onClick={() => interact("tab", ["asset"], key)} data-testid={`button-practice-defi-tab-${key}`} className={`min-h-10 border-b-2 px-3 text-[10px] font-bold uppercase tracking-[0.12em] ${tab === key ? "border-cyan-200 text-cyan-100" : "border-transparent text-white/38"}`}>{key === "landing" ? "Dashboard" : key === "lp" ? "Liquidity" : key}</button>)}
          </div>
          <div className="flex items-center gap-2"><button type="button" onClick={() => interact("search", [], !data.simulator?.search)} className="flex h-9 items-center gap-2 border border-white/[0.12] px-2.5 text-[10px] text-white/42" data-testid="button-practice-defi-search"><Search className="h-3.5 w-3.5" />Search reserves</button><DemoPill>DEMO MARKET</DemoPill></div>
        </div>
        {tab === "landing" && <div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="border border-cyan-200/15 bg-cyan-200/[0.05] p-4"><p className="text-[9px] uppercase tracking-[0.14em] text-white/35">TVL</p><p className="mt-2 text-2xl font-extrabold text-white/85">$24.71B</p><p className="mt-1 text-[10px] text-emerald-100/65">+8.4% this month</p></div><div className="border border-white/[0.1] bg-[#111923] p-4"><p className="text-[9px] uppercase tracking-[0.14em] text-white/35">Borrowing</p><p className="mt-2 text-2xl font-extrabold text-white/85">$10.00B</p><p className="mt-1 text-[10px] text-white/35">Health factor required</p></div><div className="border border-white/[0.1] bg-[#111923] p-4"><p className="text-[9px] uppercase tracking-[0.14em] text-white/35">Liquidity</p><p className="mt-2 text-2xl font-extrabold text-white/85">$14.19B</p><p className="mt-1 text-[10px] text-white/35">Available in demo market</p></div></div>}
        <div className="mt-5 border border-white/[0.11] bg-[#111923]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.09] px-4 py-3"><div><p className="text-sm font-bold text-white/82">{surface} assets</p><p className="mt-1 text-[10px] text-white/35">Select a reserve to open the action panel.</p></div><span className="text-[10px] text-white/35">Supply APY · Borrow APY</span></div>
          <div className="hidden grid-cols-[1.6fr_1fr_1fr_90px] gap-3 border-b border-white/[0.08] px-4 py-2 text-[9px] uppercase tracking-[0.12em] text-white/30 sm:grid"><span>Asset</span><span>Total supplied</span><span>APY</span><span /></div>
          <div>{reserves.map((item) => <button type="button" key={item[1]} onClick={() => interact("reserve", ["asset", "terms"], item[1])} data-testid={`button-practice-defi-reserve-${item[1]}`} className={`grid w-full grid-cols-[1fr_auto] gap-3 border-b border-white/[0.07] px-4 py-3 text-left transition hover:bg-white/[0.03] sm:grid-cols-[1.6fr_1fr_1fr_90px] ${selected === item[1] ? "bg-cyan-200/[0.05]" : ""}`}><span><span className="flex items-center gap-2 text-xs font-bold text-white/72"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-cyan-200/15 text-[9px] text-cyan-100">{item[1][0]}</span>{item[0]}</span><span className="ml-8 text-[9px] text-white/30">{item[1]} · TEST</span></span><span className="hidden text-[11px] text-white/60 sm:block">{item[2]}</span><span className="text-right text-[11px] text-emerald-100/70 sm:text-left">{tab === "borrow" ? item[4] : item[3]}</span><span className="text-right text-[10px] text-cyan-100/70 sm:text-left">Details</span></button>)}</div>
        </div>
        {selected && <div className="mt-4 border border-cyan-200/20 bg-cyan-200/[0.045] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-cyan-100/60">Reserve detail · {selected}</p><h5 className="mt-2 text-lg font-extrabold text-white/88">{surface} {selected}</h5></div>{tab === "borrow" && <div className="text-right"><p className="text-[9px] uppercase text-white/35">Health factor</p><p className="mt-1 text-lg font-extrabold text-emerald-100/80">1.84</p></div>}</div><div className="mt-4 grid gap-2 sm:grid-cols-3"><MiniBox label="Available" value="14.19B TEST" /><MiniBox label={tab === "borrow" ? "Borrow APY" : "Supply APY"} value={tab === "borrow" ? "2.05%" : "4.72%"} /><MiniBox label="Risk" value={tab === "borrow" ? "Liquidation" : "Smart contract"} /></div><div className="mt-4 flex flex-wrap gap-2"><SimButton tone="cyan" onClick={() => interact("opened", ["exit"], true)} testId={`button-practice-defi-${tab}`} icon={tab === "borrow" ? ArrowDown : Coins}>{tab === "borrow" ? "Preview borrow" : "Preview supply"}</SimButton><span className="self-center text-[10px] text-white/35">No wallet or balance required</span></div></div>}
      </div>
    </ProtocolShell>
  );
}
...
function PerpsSimulator({ data, interact }) {
  const side = data.simulator?.side || "long";
  const order = data.simulator?.order || "Limit";
  const leverage = data.simulator?.leverage || "5x";
  const targetVolume = data.simulator?.targetVolume || "1,000";
  return (
...
function Simulator({ category, data, interact }) {
  let surface;
  if (category.id === "social-quest-wl") surface = <SocialSimulator data={data} interact={interact} />;
  else if (category.id === "dex-swap") surface = <DexSimulator data={data} interact={interact} />;
  else if (category.id === "defi") surface = <DefiSimulator data={data} interact={interact} />;
  else if (category.id === "meme-launchpad") surface = <MemeSimulator data={data} interact={interact} />;
  else if (category.id === "bridge") surface = <BridgeSimulator data={data} interact={interact} />;
  else if (category.id === "perps-trading") surface = <PerpsSimulator data={data} interact={interact} />;
  else if (category.id === "nft") surface = <NftSimulator data={data} interact={interact} />;
  else if (category.id === "onchain-interaction") surface = <OnchainSimulator data={data} interact={interact} />;
  else if (category.id === "smart-contract-activity") surface = <ContractSimulator data={data} interact={interact} />;
  else surface = <RwaSimulator data={data} interact={interact} />;
  return <div className="space-y-4">{surface}{category.id !== "social-quest-wl" && <PracticeTaskList category={category} data={data} interact={interact} />}</div>;
}
...
function PracticePanel({ category, data, onUpdate, onComplete, canComplete, onPrevious, onNext, hasPrevious, hasNext }) {
  ...
  const interact = (key, steps = [], value = true) => {
    ...
    const nextValue = key === "minted" ? Number(data.simulator?.minted || 0) + 1 : value;
    const nextSimulator = key.startsWith("task:")
      ? { ...data.simulator, tasks: { ...data.simulator?.tasks, [key.slice(5)]: true } }
      : { ...data.simulator, [key]: nextValue };
    onUpdate({ simulator: nextSimulator, steps: nextSteps });
  };
